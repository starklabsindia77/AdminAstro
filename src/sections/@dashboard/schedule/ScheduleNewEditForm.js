...
ScheduleNewEditForm.propTypes = {
  isEdit: PropTypes.bool,
  currentSchedule: PropTypes.object,
};

export default function ScheduleNewEditForm({ isEdit, currentSchedule }) {
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      TimeInterval: currentSchedule?.TimeInterval || '',
      monday: currentSchedule?.monday || false,
      tuesday: currentSchedule?.tuesday || false,
      wednesday: currentSchedule?.wednesday || false,
      thursday: currentSchedule?.thursday || false,
      friday: currentSchedule?.friday || false,
      saturday: currentSchedule?.saturday || false,
      sunday: currentSchedule?.sunday || false,
      // status: currentSchedule?.status,
      // firstName: currentSchedule?.firstName || '',
      // lastName: currentSchedule?.lastName || '',
    }),
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (accessToken && isValidToken(accessToken)) {
        ...
        if(isEdit){
          // const response = await axios.put(`/user/${currentSchedule?.id}`, data);
        }else{
